...
public class UserResourceIntTest extends SimulationRuntimeIntegrationTest {
	...
	public void findPage() throws Exception {
		...
		// Get all the users
		restUserMockMvc
			.perform(get(DEFAULT_API_URL).param(PageModel.F_SORTS, "a.created_date,desc")
				.param("available", CommonConstants.STR_YES)
	...
	public void updateUserExistingEmail() throws Exception {
		...
		restUserMockMvc
			.perform(post(DEFAULT_API_URL).contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(managedUserVM)))
			.andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value(ResponseCode.BASE_VALID_PARAM.getCode()))
			.andExpect(jsonPath("$.message").isNotEmpty());
		UserDo testUserDo = userService.getById(updatedUserDo.getId());
		assertThat(testUserDo.getEmail()).isEqualTo(DEFAULT_EMAIL);
	...
	public void updateUserExistingUsername() throws Exception {
		...
		managedUserVM.setRoleIdList(roleService.findListByUserId(updatedUserDo.getId()).stream().map(roleDo -> roleDo.getId()).collect(Collectors.toList()));
		managedUserVM.setId(updatedUserDo.getId());
		restUserMockMvc
			.perform(post(DEFAULT_API_URL).contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(managedUserVM)))
