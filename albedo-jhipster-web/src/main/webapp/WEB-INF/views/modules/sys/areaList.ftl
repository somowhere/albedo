    ...
    <div class="col-md-10">
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                ...
                <div class="actions">
                    <div class="btn-group">
                        <#if SecurityUtil.hasPermission('sys_area_edit')><a id="add-area" class="btn red dialog" href="javascript:void(0);" 
                        data-url="${ctx}/sys/area/edit" data-reload-after="refreshTreeArea" data-is-modal="true" data-modal-width="950" data-table-id="#data-table-area">
            ...
            <div class="portlet-body">
            	<form class="form-inline form-search" area="form">
            		<input id="parentId" name="parentId" type="hidden" operate="eq" searchItem="searchItem"/>
						<div class="form-group">
							...
							<input type="text" class="form-control" searchItem="searchItem" id="name" name="name" value="${(area.name)!}" operation="like" htmlEscape="false" maxlength="32" placeholder="...">
						</div>
						<div class="form-group">
					                             <label class="input-label" for="code">区域编码</label>
							<input type="text" class="form-control" searchItem="searchItem" id="code" name="code" value="${(area.code)!}" operation="like" htmlEscape="false" maxlength="32" placeholder="...">
						</div>
					<div class="form-group form-btn">
                        <button class="btn btn-sm green btn-outline filter-submit-table-area margin-bottom" type="button"><i class="fa fa-search"></i> 查询</button>
                ...
                <table class="table table-striped table-bordered table-hover dataTable no-footer dt-responsive" id="data-table-area">
                    <thead>
...
<script type="text/javascript">
                ...
                dataTable: {
					...
					<#if SecurityUtil.hasPermission('sys_area_edit,sys_area_delete,sys_area_lock')>, 
                        { orderable: false, data: function ( row, type, val, meta ) {
                    		...
                    		<#if SecurityUtil.hasPermission('sys_area_delete')>+'<a href="javascript:void(0);" class="confirm" data-table-id="#data-table-area" data-reload-after="refreshTreeArea" data-method="delete" data-title="你确认要删除选中的区域管理吗？" data-url="${ctx}/sys/area/delete/'+row.id+'"><i class=\"fa fa-lg fa-trash-o font-red-mint\" title=\"删除\"></i></a></span>'</#if>+'<\span>';
                        	return data;
                        }
                        }</#if>
                    ]
    ...
    var nodeId = albedo.getUserCookie("tree_area_select_node_id"), nodeId = (nodeId)? nodeId : 1, addUrl = $("#add-area").attr("data-url");;
