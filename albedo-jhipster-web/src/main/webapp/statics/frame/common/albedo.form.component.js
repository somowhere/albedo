...
//				$container.animate({scrollTop: scorll},500)
	...
	var treeShow = function($thiz){
	    ...
	    url = $thiz.attr("_url") ? $thiz.attr("_url") : "", checked = $thiz.attr("_checked") == "true" ? true : false,
		...
		var tree, setting = {
			view:{selectedMulti:false, fontCss:function(treeId, treeNode) {
			...
			callback:{
				beforeClick: function(treeId, treeNode){
					if(allowCancelSelect && tree && tree.getSelectedNodes()[0] && tree.getSelectedNodes()[0].id == treeNode.id){
						...
						return false;
					}else if(notAllowSelect){ return false;
		...
		refreshTree();
		$thiz.prev("div").find("input").bind("focus", function(){_treeSearchInputFocusKey($(this));}).bind("blur", function(){_treeSearchInputBlurKey($(this), tree);})
		.bind("change keydown cut input propertychange", function(){_treeSearchNode($(this), tree);});
	...
	var handleTreeSelect = function ($thiz, $thizVal){
	    ...
	    url = $thiz.attr("_url") ? $thiz.attr("_url") : "", module = $thiz.attr("_module") ? $thiz.attr("_module") : "",
	    checked = $thiz.attr("_checked") == "true" ? $thiz.attr("_checked") : "", extId = $thiz.attr("_extId") ? $thiz.attr("_extId") : "",
	    nodesLevel = $thiz.attr("_nodesLevel") ? $thiz.attr("_nodesLevel") : "", title = $thiz.attr("_title") ? $thiz.attr("_title"): "",
	    value = $thizVal.val() ? $thizVal.val() : "", allowClear = $thiz.attr("_allowClear") ? $thiz.attr("_allowClear") : "",
		...
		var setting = {view:{selectedMulti:false},check:{enable:checked,nocheckInherit:true},
				data:{simpleData:{enable:true}},
				view:{
		...
		}).bind("change keydown cut input propertychange", function(){_treeSearchNode($(this), $.fn.zTree.getZTreeObj("tree-"+name));});
	...
	var handleGridSelect = function ($thiz, $thizVal){
	    ...
	    var name = $thiz.attr("name"), id=$thizVal.attr("id"), colNames = $thiz.attr("_colNames"), gridOptions = $thiz.attr("_gridOptions"), colModel = $thiz.attr("_colModel"), searchFormId = $thiz.attr("_searchFormId"),
	    url = $thiz.attr("_url") ? $thiz.attr("_url") : "",
	    checked = $thiz.attr("_checked") == "true" ? true : false, disableDblClickRow = $thiz.attr("_disableDblClickRow") == "true" ? true : false,
	    extId = $thiz.attr("_extId") ? $thiz.attr("_extId") : "",
	    width = $thiz.attr("_width") ? $thiz.attr("_width") : 920,height = $thiz.attr("_height") ? $thiz.attr("_height") : 735,
	    pageSize = $thiz.attr("_pageSize") ? $thiz.attr("_pageSize") : 10, title = $thiz.attr("_title") ? $thiz.attr("_title"): "",
	    value = $thizVal.val() ? $thizVal.val() : "", allowClear = $thiz.attr("_allowClear") ? $thiz.attr("_allowClear") : "",
	    ...
	    try{eval("gridOptions="+gridOptions);}catch(e){}
	    	...
	    	$modal.find("#data-table-"+id+' tbody').off('click', 'tr').on('click', 'tr', function () {
	    		...
	    		if(checked && $(this).attr("class").indexOf("selected")!=-1){ //取消 选择
	    			...
	    			for(var index = 0 ; index < rsData.data.length; index++){
	        			...
	        			if(item.id == id){
	        				names.removeByValue(eval("item."+showProperty));nodes.removeByValue(item);break;
	        			}
	        		}
	    		}else{
	    			ids.push(id);
...
//		App.initSlimScroll('.scroller');
    ...
    var handleFileUpload = function ($tagert) {
    	...
    	var clearVal = function(){
			...
			var tempVal = ","+$file.val()+","; tempVal=tempVal.replace($(this).attr("img-value")+",", "");
    	...
    	$tagert.length>0 && $tagert.fileupload && $tagert.each(function(){
    		var options = $(this).attr("options"), $parent = $(this).parents(".fileinput"), multiple = $(this).attr("multiple"), showType = $(this).attr("showType");
