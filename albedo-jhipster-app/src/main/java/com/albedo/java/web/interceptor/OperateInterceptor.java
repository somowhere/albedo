...
public class OperateInterceptor implements HandlerInterceptor {
	...
	public static final String SGINKEY = "ypjy-card";
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (flag) {
			...
			if (handler instanceof HandlerMethod) {
				...
				if(flag){
					// 接口权限过滤
//					ApiAuthority appAuthority = handlerMethod.getMethodAnnotation(ApiAuthority.class);
//					if (PublicUtil.isNotEmpty(params) && appAuthority != null) {
//						String myAuth = SecurityUtil.sign(params, SGINKEY);
//						String auth = request.getParameter("auth");
//						if (auth == null || !myAuth.equals(auth.toLowerCase())) {
//							BaseResource.writeStringHttpResponse("", response);
//							BaseResource.writeJsonHttpResponse(Message.createWarn("请更新最新版本,否则部分功能无法使用哦"), response);
//							flag = false;
//						}
//					}
				}
