...
public class FastDfsFileStrategyImpl extends AbstractFileStrategy {
	...
	protected void uploadFile(FileDo fileDo, MultipartFile multipartFile, String bucket) throws Exception {
		StorePath storePath = storageClient.uploadFile(multipartFile.getInputStream(), multipartFile.getSize(), fileDo.getSuffix(), null);
		fileDo.setUrl(fileProperties.getFastDfs().getUrlPrefix() + storePath.getFullPath());
		fileDo.setBucket(storePath.getGroup());
		fileDo.setPath(storePath.getPath());
		fileDo.setStorageType(FileStorageType.FAST_DFS);
	}
