...
public class AliFileChunkStrategyImpl extends AbstractFileChunkStrategy {
	public AliFileChunkStrategyImpl(FileRepository fileMapper, FileServerProperties fileProperties) {
	...
	protected void copyFile(FileDo fileDo) {
		...
		String sourceObjectName = fileDo.getPath();
		String fileName = UUID.randomUUID() + StrPool.DOT + fileDo.getSuffix();
		String destinationObjectName = StrUtil.subBefore(fileDo.getPath(), "/" + fileDo.getUniqueFileName(), true) + StrPool.SLASH + fileName;
		...
		metadata.setContentDisposition("attachment;fileName=" + fileDo.getOriginalFileName());
		metadata.setContentType(fileDo.getContentType());
		...
		fileDo.setUrl(StrUtil.replace(url, "\\\\", StrPool.SLASH));
