...
public abstract class AbstractFileStrategy implements FileStrategy {
	 ...
	 */
	@Override
	public FileDo upload(MultipartFile multipartFile, String bucket, String bizType) {
		try {
			...
			FileDo fileDo = FileDo.builder()
				...
				.build();
			uploadFile(fileDo, multipartFile, bucket);
			return fileDo;
		} catch (Exception e) {
			log.error("ex", e);
	 ...
	 * @throws Exception 异常
	 */
	protected abstract void uploadFile(FileDo fileDo, MultipartFile multipartFile, String bucket) throws Exception;
	...
	protected String getUniqueFileName(FileDo fileDo) {
		return new StringJoiner(StrPool.DOT)
			.add(UUID.randomUUID().toString().replace("-", ""))
			.add(fileDo.getSuffix()).toString();
	}
}
