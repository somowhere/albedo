            ...
            <form id="genTable-form" action="${ctx}/gen/genTable/edit" method="post"
                ...
                <div class="form-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-5">
                            <input type="text" name="name" value="${(genTable.name)! }" maxlength="64" data-required="1" class="form-control required" /> 
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-5">
                            <input type="text" name="comments" value="${(genTable.comments)! }" maxlength="64" data-required="1" class="form-control required" /> 
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-5">
                            <input type="text" name="className" value="${(genTable.className)! }" maxlength="64" data-required="1" class="form-control required" /> 
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                        <textarea class="summernote form-control" name="description" rows="5">${(genTable.description)! }</textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <table id="contentTable" class="table table-striped table-bordered table-condensed">
							<thead><tr><th title="数据库字段名">列名</th><th title="默认读取数据库字段备注">说明</th><th title="数据库中设置的字段类型及长度">物理类型</th><th title="实体对象的属性字段类型">Java类型</th>
								...
								<th title="是否是数据库主键">主键</th><th title="字段是否可为空值，不可为空字段自动进行空值验证">可空</th><th title="字段是否唯一，唯一空字段自动进行唯一性验证">唯一</th><th title="选中后该字段被加入到insert语句里">插入</th>
							...
							<#list genTable.columnList as column>
								<tr<#if column.status == -1> class="error" title="已删除的列，保存之后消失！"</#if> >
									<td nowrap>
										<input type="hidden" name="columnFormList[${(column_index)!}].id" value="${(column.id)!}"/>
										<input type="hidden" name="columnFormList[${(column_index)!}].status" value="${(column.status)!}"/>
										<input type="hidden" name="columnFormList[${(column_index)!}].genTable.id" value="${(column.genTable.id)!}"/>
										<input type="hidden" name="columnFormList[${(column_index)!}].name" value="${(column.name)!}"/>${(column.name)!}
									</td>
									<td>
										<input type="text" name="columnFormList[${(column_index)!}].comments" value="${(column.comments)!}" maxlength="200" class="form-control required input-small"/>
									</td>
									<td nowrap>
										<input type="hidden" name="columnFormList[${(column_index)!}].jdbcType" value="${(column.jdbcType)!}"/>${(column.jdbcType)!}
									</td>
									<td>
									...
									<td>
										<input type="text" name="columnFormList[${(column_index)!}].javaField" value="${(column.javaField)!}" maxlength="200" class="required form-control input-small"/>
									</td>
									<td>
										<input type="checkbox" name="columnFormList[${(column_index)!}].isPk" value="1" <#if column.isPk == 1>checked='checked'</#if>/>
									</td>
									<td>
										<input type="checkbox" name="columnFormList[${(column_index)!}].isNull" value="1" <#if column.isNull == 1>checked='checked'</#if>/>
									</td>
									<td>
										<input type="checkbox" name="columnFormList[${(column_index)!}].isUnique" value="1" <#if column.isUnique == 1>checked='checked'</#if> />
									</td>
									<td>
										<input type="checkbox" name="columnFormList[${(column_index)!}].isInsert" value="1" <#if column.isInsert == 1>checked='checked'</#if>/>
									</td>
									<td>
										<input type="checkbox" name="columnFormList[${(column_index)!}].isEdit" value="1" <#if column.isEdit == 1>checked='checked'</#if>/>
									</td>
									<td>
										<input type="checkbox" name="columnFormList[${(column_index)!}].isList" value="1" <#if column.isList == 1>checked='checked'</#if>/>
									</td>
									<td>
										<input type="checkbox" name="columnFormList[${(column_index)!}].isQuery" value="1" <#if column.isQuery == 1>checked='checked'</#if>/>
									</td>
									...
									<td>
										<input type="text" name="columnFormList[${(column_index)!}].dictType" value="${(column.dictType)!}" maxlength="200" class="input-xsmall form-control"/>
									</td>
									<td>
										<input type="text" name="columnFormList[${(column_index)!}].sort" value="${(column.sort)!}" maxlength="200" class="form-control required input-mini digits"/>
									</td>
								</tr>
							</#list>
							</tbody>
